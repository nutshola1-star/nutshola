      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-end mb-4 sm:mb-8">
          <div>
            ...
            {/* Main Heading */}
            <h2 className="text-xl sm:text-3xl md:text-4xl font-serif font-bold text-[#3A393D]">
              Shop by Category <span className="text-xl sm:text-2xl">🐝</span>
            </h2>
          </div>
        ...
        {catfetchLoading ? (
          ...
          <div className="flex gap-2 sm:gap-6 overflow-hidden">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex flex-col items-center shrink-0 w-[calc(50%-4px)] sm:w-36 md:w-48">
              ...
              {[...categories, ...categories, ...categories].map((category, index) => (
                ...
                >
                  ...
                  <div className="relative w-[calc(100%-4px)] aspect-square sm:w-32 sm:h-32 md:w-44 md:h-44 rounded-full bg-[#f8f9f8] flex items-center justify-center p-2 sm:p-4 transition-all duration-300 group-hover/card:shadow-lg">
                    {category.image?.url && (
                      <Image
                        ...
                        sizes="(max-width: 640px) 45vw, (max-width: 768px) 128px, 176px"
                      />
                    )}
                  </div>
                  ...
                  <div className="relative z-10 mt-[-10px] sm:mt-[-18px] md:mt-[-20px]">
                    <span className="inline-block px-2 sm:px-5 py-1 sm:py-2 bg-[#559F34] text-white rounded-full text-[10px] sm:text-sm font-semibold shadow-md group-hover/card:bg-[#7ECB2A] transition-colors duration-300 whitespace-nowrap">
                      {category.name}
                      {category.bengaliName && (
                        <span className="hidden sm:inline"> ({category.bengaliName})</span>
                      )}
                    </span>
                  </div>
                </Link>
